...
## [2026-07-24] Day 3 知识点总览（整理）

> 以下把今天零散的"易混点补充"归类为一张清单，方便复盘。详细讲解见上方对应段落。

### 一、Day 3 主线（函数）
- **四参数顺序铁律**（def 括号里的顺序）：必选位置 → 默认 → `*args` → 关键字-only → `**kwargs`；写反直接 SyntaxError。`**kwargs` 永远最后。
- **`*args` / `**kwargs` 收集 vs 解包方向相反**：
  - def 里 `def f(*args)` = 收口袋（多传的位置参数打包成元组）
  - 调用里 `f(*lst)` = 拆包裹（列表/元组拆成多个位置参数）；`f(**d)` 拆字典成关键字参数
- **lambda**：`lambda 参数: 表达式`，自动 return，只能一个表达式；冒号前是**参数**（不是函数名），`u`/`x` 只是占位符、每次传入不同元素。
- **默认参数陷阱**：默认参数绝不用可变对象（list/dict），用 `None` + 函数内初始化。
- **重写 Java 工具**：`is_blank(s, trim=True)` = `s is None or s.strip() == ""`，对应 Java `StringUtils.isBlank`；`strip()` 对应 `trim()`，`== ""` 对应 `isEmpty()`。

### 二、数据结构（Day 3-4 交叉）
- **list `[]`**：有序、可重复、可下标、可变。
- **tuple `()`**：有序、可重复、可下标、**不可变**（*args 用它装参数，安全只读）。
- **set `{}`**：无序、不重复、不能下标、空集合写 `set()`；支持 `&`/`|`/`-` 集合运算；`in` 判断 O(1)。
- **dict**：`items()` 返回键值对 `(k,v)`，`keys()` 取键，`values()` 取值；`items()` 返回"视图"不是列表，要 `list()`。

### 三、内置函数
- **sorted**：返回新列表（不改原），`key=` 指定排序键，`reverse=True` 降序。
- **map** / **filter**：返回**迭代器**（惰性），要看结果必须 `list()`，且**只能消费一次**。

### 四、机制概念
- **迭代器**：一次性水龙头，`next()` 取下一个，取尽抛 `StopIteration`；`for` 循环底层就是 `iter()`+`next()`。
- **惰性 laziness**：`map`/生成器(`yield`)/`filter` 创建时不算，消费时才逐个算（对比列表推导 eager 立刻全算）。

### 五、工程规范（测试代码基本功）
- **docstring**：函数/类第一行三引号字符串，存于 `__doc__`，`help()` 可读；比 `#` 注释更"活"（能被工具读取生成文档）。
- **验证三方式**：①直接打印 ②期望对照（推荐，自动 OK/FAIL）③`assert`（Day15 pytest 底层）。
- **`if __name__ == "__main__":`**：保证验证代码只在直接运行时跑，被 import 时不跑。
- **f-string 格式**：`{x!r}` 用 repr 显示（带引号/None 清晰）；`{x:6}` 占 6 宽对齐。

### 六、今日易踩坑（必记）
- `for...else` 的 `else` 只在**循环没被 break 打断**时执行（break 是 else 死敌）。
- 定义函数 vs 调用函数：`def f(*args)` 是定义（args 是占位符）；`f(1,2,3)` 是调用（数字是实参）。
- 字符串不可变：`s.strip()` 返回新串，要 `s = s.strip()` 接住。
